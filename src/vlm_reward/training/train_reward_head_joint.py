"""
Jointly fine-tune the Qwen vision encoder and ManiSkill reward head.
"""
...
from vlm_reward.models.reward_predictor import preprocess_images
...
def evaluate(
    ...
    for bin_index, label in enumerate(labels):
        ...
        if not torch.any(mask):
            raise ValueError(f"Evaluation data contains no examples in reward bin {label}")
    ...
    for source in ["offline", "online"]:
        ...
        if not torch.any(mask):
            raise ValueError(f"Evaluation data contains no {source} examples")
...
def load_trainable_model(
    ...
    extras = torch.load(
        Path(adapter_dir) / "contrastive_extras.pt",
        map_location="cpu",
        weights_only=True,
    )
...
def build_joint_dataloaders(
    ...
    """
    Build deterministic weighted training and sequential evaluation loaders.
    """
...
def joint_training_state(
    args: argparse.Namespace,
    adapted_model: PeftModel,
    pooler: AttentionalPooler,
    head: RewardHead,
    ...
    *,
    ...
    """
    Build a complete resumable joint fine-tuning checkpoint.
    """
    return {
        "checkpoint_format_version": 2,
        "checkpoint_type": "joint_reward_training_state",
...
def train_joint_reward_model(
    ...
    *,
    ...
    """
    Fine-tune vision LoRA, pooler, and reward head and restore the best epoch.
    """
    vision_lora_parameters = [
    ...
    trainable_parameters = vision_lora_parameters + list(pooler.parameters()) + list(head.parameters())
    best_path = output_dir / "best_joint_training_state.pth"
    ...
    if initial_metrics["prediction_std"] < args.minimum_prediction_std:
        raise RuntimeError(
            "The initial reward model is already collapsed: "
            f"prediction_std={initial_metrics['prediction_std']:.8f}"
        )
    ...
    print_metrics("Initial validation", initial_metrics)
    print(f"Initial selection score: {best_score:.6f}")
    torch.save(
        joint_training_state(
            args,
            adapted_model,
            pooler,
            head,
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch_index, (pixel_values, image_grid_thw, rewards, _, _) in enumerate(
            ...
            if not torch.isfinite(unscaled_loss):
                raise FloatingPointError(
                    f"Non-finite joint reward loss in epoch {epoch}, batch {batch_index}"
                )
            (unscaled_loss / args.gradient_accumulation_steps).backward()
        ...
        print(
            ...
            f"val_online_mae={val_metrics['mae_online']:.6f} | "
...
def main() -> None:
    ...
    adapter_path = Path(args.adapter_dir).expanduser()
    reward_head_path = Path(args.reward_head_checkpoint).expanduser()
    if not adapter_path.is_dir():
        raise FileNotFoundError(adapter_path)
    if not reward_head_path.is_file():
        raise FileNotFoundError(reward_head_path)
    ...
    train_loader, val_loader, test_loader = build_joint_dataloaders(
        args,
        processor,
        device,
        train_dataset,
    ...
    best_state = train_joint_reward_model(
        args,
    ...
    reward_head_training_metadata = {
        "checkpoint_format_version": 2,
        "checkpoint_type": "reward_head_training_state",
        "task": args.task,
        "model_id": args.model_id,
        "adapter_dir": relative_artifact_path(
            adapter_output_dir,
    ...
    torch.save(
        reward_head_inference_checkpoint(reward_head_training_metadata),
        reward_head_output_path,
    )
