"""
Evaluate normalized ManiSkill reward predictions.
"""
import argparse
import json
...
def mae(values: List[float]) -> float:
    if not values:
        raise ValueError("Cannot calculate MAE for an empty value set")
...
def main() -> None:
    ...
    for row in rows:
        ...
        if len(generated) != len(target):
            raise ValueError(
                f"{row['id']} has {len(generated)} predictions for {len(target)} targets"
            )

        for pred_reward, target_reward in zip(generated, target):
            pred_value = float(pred_reward)
            target_value = float(target_reward)
            if not np.isfinite(pred_value) or not 0.0 <= pred_value <= 1.0:
                raise ValueError(
                    f"{row['id']} contains invalid predicted reward: {pred_value}"
                )
            if not np.isfinite(target_value) or not 0.0 <= target_value <= 1.0:
                raise ValueError(
                    f"{row['id']} contains invalid target reward: {target_value}"
                )
