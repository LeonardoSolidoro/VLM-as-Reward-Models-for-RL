"""Original CrossQ progress-as-reward experiment.

This runnable trial was contributed by a project collaborator and established
the CrossQ starting point for the final visual-reward experiment.  Its VLM
progress rewards proved too noisy to train a reliable policy.  Shared CrossQ
and environment components now live in :mod:`vlm_reward.rl`; this command
retains the original experiment-specific annotation and training flow.
"""
...
# Ensure the shared package is importable when this file is run directly.
SRC_ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))
if SRC_ROOT not in sys.path:
    sys.path.insert(0, SRC_ROOT)

from vlm_reward.training.multimodal import build_user_content
from vlm_reward.training.progress_format import build_frames_list
from vlm_reward.rl.core import (
    PolicyNetwork,
    QNetwork,
    SimpleReplayBuffer,
    set_seed,
)
from vlm_reward.rl.environments import (
    get_state_dict,
    update_wrist_follow_camera,
)
from vlm_reward.rl.evaluation import evaluate_policy
