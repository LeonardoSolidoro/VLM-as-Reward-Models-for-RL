"""Collaborator-provided trajectory generator for the original RL trial.

This historical command is retained because it produced the expert, partial,
random, and regressing trajectories used while testing autoregressive VLM
progress as a reward. The maintained normalized-reward data pipeline lives in
``vlm_reward.data.generate_reward_data``.
"""
...
DEMO_ROOT = os.path.join(PROJECT_ROOT, "demos")
